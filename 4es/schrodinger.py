...
def V(xi):
    ...
    #return xi**2 -5            #potenziale armonico con minimo negativo
...
'''
n       = 14000
nmatch  = 7000 #dove xi=rad(2*eps) ossia V(x)=E, pasta cercare dove funzione psi non piccola
xi      = np.linspace(-7.,7,n)
h       = xi[1]-xi[0]

'''
  Numerov chiamando la funzione da sinistra a destra
'''
...
'''
  Numerov chiamando la funzione da destra a sinistra
'''
#epsilon = 0.5   # o altra energia di stato definito eps = (n+1/2)
...
def findAllE(energiamin, energiamax,step,tol):
    energie = np.array([])  #array contenente le energie esatte trovate
    a = energiamin          #estremo inferiore dell'intervallo in cui controllo il cambio segno della derivata
...
print("Cacolare spettro energia? 0 no, 1 si ")
bool = input()
if int(bool) != 0:
    print("Calcolare spettro tra a e b")
    enmin = float(input())
    enmax = float(input())
    print("Calcolo spettro dell'energia...")
    print("Energie: ", findAllE(enmin, enmax, 0.1, 1e-5));                  #spettro dell'energia

#############################CALCOLO PSI
print("Psi con energia tra a e b: ")
e1=float(input())
e2=float(input())

e = findE(e1, e2, 0.00001)  #tra che estremi imporre energia e precisione
