...
class OdeSolver{
 public:
  ...
  double           DeltaT();
  void             Step();//f apassare da un t ad uno successvio
 ...
 private:
  string   m_method;
  vector<MatPoint> m_p; //vector con pos e vel ad un t
  double   m_t,m_h; 
  Vector3  m_A(unsigned int i, double t, vector<MatPoint>); //accel corpo i
};
